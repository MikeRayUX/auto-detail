class Users::Dashboards::NewOrderFlow::PickupEstimationsController < ApplicationController
  protect_from_forgery with: :exception
  before_action :ensure_eligible_to_order, only: %i[index]
  before_action :authenticate_user!
  before_action :business_open?
  before_action :washers_available?

  # users_dashboards_new_order_flow_pickup_estimations_path
  # /users/dashboards/new_order_flow/pickup_estimations
  def index
    @minutes = rand(55..65)
  ...
  def ensure_eligible_to_order
    unless current_user.address.present? && 
           current_user.within_region?
