class Executives::Dashboards::Commercial::NewClientsController < ApplicationController
  before_action :authenticate_executive!

  layout 'executives/no_nav_layout'
  
  # new_executives_dashboards_commercial_new_client_path GET
  def new
    @new_client = NewClient.new
  end

  # executives_dashboards_commercial_new_clients_path POST
  def create
    @new_client = NewClient.new(new_client_params.merge(
      card_params
    ))
    ...
    if @new_client.valid?
      ...
      @client.create_stripe_customer!(card_params)

      @client.send_new_client_email!
      redirect_to executives_dashboards_commercial_clients_path, flash: {
        notice: 'Client Created Successfully!'
      }
    else
      flash[:errors] = @new_client.errors.full_messages.first
      render :new
    end

    rescue Stripe::StripeError => e
      flash[:errors] = "Stripe Error: #{e}"
      render :new
  end
